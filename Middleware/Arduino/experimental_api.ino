...
public:
  ...
  unsigned long getDOS() { return DOS; }
...
// This is for splitting the API String
#define SPTR_SIZE 20
char *sPtr[SPTR_SIZE];

// This function splits the string at spaces and stores them in array
...
// For starting, stopping vibration in loop
...
void loop() {
  ...
  // Interpret API Command
  if (Serial.available()) {
    apiCommand = Serial.readStringUntil('\n');
    Serial.println("API Command : " + apiCommand);
    running = true;
    // Split String
    int N = separate(apiCommand, sPtr, SPTR_SIZE);
    for (int n = 0; n < N; n++) {
      Serial.println(sPtr[n]);
    }
    if (sPtr[0] == 'act') {
      running = true;
      Serial.println("ACTTT");
    }
  }
  ...
  endtime = Motors[1].getPS() + Motors[0].gettotalTime();

  //   Serial.println(endtime);

  // Plot the intensity when the all the motors not resting
  //   unsigned long sum =
